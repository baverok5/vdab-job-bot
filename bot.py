...
CANDIDATE_ONELINE = (
    ...
    "specialist-degree roles, NOT 5+ years required. English + Turkish, Dutch "
    "writing B1 / speaking A2+ (conversational Dutch OK), no French."
)
...
Do NOT rewrite, reorder or tailor the CV itself. One fixed CV goes out with every
application — English as written, Dutch as a straight translation of it — so the
CV below is background for the letter, not something to reshape.
...
def write_letter(job_id, url, job_text, apply_email, cv_text, info=None):
    ...
    save_json(out, {
        ...
        "fmt": 4,   # 4 = no per-job CV; the app serves the one real CV
    })
    return True


def has_letter(job_id):
    """A letter counts as complete once it has a cover letter in the posting's
    own language.
    ...
    rewrites them in the posting's OWN language. Until a letter's turn comes the
    app keeps showing the bilingual text, which still works.

    The CV used to be part of this test, back when each letter carried its own
    rewritten copy. It no longer does — there is one CV, served by the app from
    cv.en.md / cv.nl.md — so requiring it here would send every letter ever
    written back through the generator for nothing."""
    p = os.path.join(PREPARED_DIR, f"{job_id}.json")
    try:
        d = json.load(open(p, encoding="utf-8"))
        return bool(d.get("cover_letter")) and d.get("fmt", 0) >= 3
    except (FileNotFoundError, json.JSONDecodeError):
        return False
...
- GOAL FIELD: digital marketing / SEO & GEO / content / WordPress & web / web
  ...
  developer (AngularJS/JavaScript). Also writes Python automation and has an
...
- Languages: English (fluent), Turkish (native), Dutch writing B1 / speaking A2+
  (taking classes, improving), no French.

EXPERIENCE RULE (important): count roughly 2-3 years of relevant web/SEO/
marketing experience. Jobs asking for UP TO ~3 years are fine. Jobs asking 4-5
...
def main():
    ...
    # The CV now shows what the old one left out: ~2 years as Web & SEO
    # Specialist at Knowledge Experts before the Episto role. Every job turned
    # down for wanting 2-3 years was judged against a CV claiming four months,
    # so those rejections say nothing about this candidate. Read them again.
    if not jobs.get("new_cv_exp_requeue_v1"):
        exp_rx = re.compile(r"\b[2-9]\s*(\+|plus)?\s*(-|to|a|à)?\s*\d*\s*"
                            r"(year|jaar|jr\b|years')|years of (relevant |"
                            r"professional |hands-on )?experience|"
                            r"jaar (relevante )?ervaring|experience wall|"
                            r"only ~?4 months|four months|more experience than",
                            re.I)
        again = {x["id"] for x in jobs.get("rejected", [])
                 if exp_rx.search(f"{x.get('reason','')} {x.get('why_bad','')}")}
        again |= {i for i, v in (jobs.get("verdicts") or {}).items()
                  if v[0] == "n" and exp_rx.search(v[2] or "")}
        # Only in-field titles. The raw match is 1,297 jobs, but most are
        # warehouse, French-only or trade roles whose reason merely happened to
        # mention a number of years — re-reading those changes nothing and would
        # hold up the queue for days. 323 marketing / SEO / web / content ones
        # are the jobs the missing two years actually blocked.
        by_id = {x["id"]: x for x in jobs.get("listing", [])}
        again = {i for i in again & set(by_id)
                 if title_priority(by_id[i].get("title", "")) <= 2}
        if again:
            jobs["rejected"] = [x for x in jobs.get("rejected", [])
                                if x["id"] not in again]
            for _i in again:
                (jobs.get("verdicts") or {}).pop(_i, None)
            seen -= again
            print(f"  new CV: re-reading {len(again)} in-field job(s) turned down "
                  f"for years of experience the old CV did not show")
        jobs["new_cv_exp_requeue_v1"] = sorted(again)
